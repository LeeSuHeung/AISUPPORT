...
이 저장소를 비공개 Git 원격에 올린 뒤 새 PC에서 복제합니다.
...
설치 프로그램은 기존 파일의 내용이 다를 경우 같은 위치에 `.backup-날짜` 파일을 만든 뒤 설치합니다. 전역 `AGENTS.md`의 다른 지침은 보존하고, 이 팀의 관리 구역만 추가하거나 갱신합니다.
...
원본은 `bundle` 폴더에서 관리합니다. 한 PC에서 수정해 Git에 올린 뒤, 다른 PC에서 `git pull`하고 설치 스크립트를 다시 실행하면 갱신됩니다.
